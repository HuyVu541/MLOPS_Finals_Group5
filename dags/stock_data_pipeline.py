...
params = {
    "golden_database_uri": Param(default_golden_uri, type="string", title="Golden DB URI",
                                 description="SQLAlchemy URI for the Golden PostgreSQL database."),
    "feature_store_uri": Param(default_feature_uri, type="string", title="Feature DB URI",
                               description="SQLAlchemy URI for the Feature Store PostgreSQL database."),
    ...
    # "unique_key_column": Param("Time", type="string", title="Unique Key Column(s)",
    # description="Comma-separated if multiple keys."),
    # Allow comma separated for multiple keys
}
...
with DAG(
    ...
    def task_ingest_data(**context):
        ...
        print(
            f"Ingestion task finished. Data appended to Golden DB (URI used), table {golden_table_name}")
    ...
    def task_engineer_features(**context):
        ...
        # Feature engineering overwrites the feature table each time based on the
        # current golden data
        ...
        print(f"Feature engineering task finished. \
              Features stored in Feature Store DB (URI used), table {feature_store_table_name}")
